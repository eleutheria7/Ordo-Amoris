    ...
    <header className="fixed w-full top-0 bg-white shadow-md z-50">
      <nav className="container mx-auto px-6 py-4 flex justify-between items-center">
        ...
        <div className="hidden md:flex space-x-8 text-2xl items-center">
          {["Sobre", "Recomendações", "Local", "Contato"].map((item) => (
            <ScrollLink
              key={item}
              href={`#${item}`}
          ...
          <Link
            href="/formulario"
            className="bg-blue-600 text-white px-4 py-4 rounded-full hover:bg-blue-700 transition-colors text-sm font-semibold flex items-center justify-center disable"
          >
            INSCRIÇÕES
          </Link>
        </div>
        ...
        <div className="md:hidden">
          ...
          {isOpen && (
            <div className="flex flex-col gap-4 top-full left-0 right-0 bg-white shadow-lg p-4 py-2 px-4 pb-8 text-gray-600 hover:bg-gray-100 text-2xl">
              {["Sobre", "Recomendações", "Local", "Contato"].map(
                (item) => (
                  <ScrollLink
                    key={item}
                    href={`#${item}`}
              ...
              <Link
                href="/formulario"
                className="bg-blue-600 text-white px-4 py-4 rounded-full text-sm items-center justify-center hidden disable"
              >
